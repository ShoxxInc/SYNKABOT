...
async def test_fetch_latest_non_wp_blog_post(blog_dict, post_slug, link):
    ...
    with patch.object(aiohttp.ClientSession, "get") as mock_get:
        ...
        lastest_route, print_string = await fetch_latest_non_wp_blog_post(
            aiohttp.ClientSession(), blog_dict
        )
...
@pytest.mark.parametrize(
    "blog_dict, post_id, title, date, link",
    TEST_CASES_WP
)
@pytest.mark.asyncio
async def test_fetch_latest_wp_blog_post(
    blog_dict,
    post_id,
    title,
    date,
    ...
    mock_get.assert_called_once_with(
        blog_dict["url"] + "/posts", params={"per_page": 1, "orderby": "date"}
    )
    mock_response.raise_for_status.assert_called_once()
